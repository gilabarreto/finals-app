...
router.post("/add", authorization, async (req, res) => {
  ...
  checkForArtist(req.body.artistId, req.body.artistName, req.body.image).then(
    (result) => {
      ...
      pool
        .query(`SELECT * FROM favourites WHERE artist_id = $1 AND user_id = $2`, [result, req.user.id])
        .then((favresult) => {
...
router.get("/", authorization, async (req, res) => {
  pool.query(`SELECT artists.artistname, artists.artistimage 
            FROM artists 
            JOIN favourites 
              ON artists.id = favourites.artist_id 
            JOIN users 
              ON users.id = favourites.user_id 
            WHERE users.user_email = $1`,[req.user.user_email])
            .then((sqlresults) => {
